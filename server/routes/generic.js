            ...
            if (Controller.hasOwnProperty(handler)) {
                if (route.hasOwnProperty('active') && route.active) {
                    if (route.hasOwnProperty("middleware") && route.middleware.length > 0) {
                        router[route.method.toLowerCase()](route.path, route.middleware, Controller[handler]);
                    } else {
                        router[route.method.toLowerCase()](route.path, Controller[handler]);
                    }
                }
            } else {
                throw new Error(`handler '${handler}' is not defined in the generic controller`);
            }
        });
    } else {
        throw new Error("defaulRoutes parameter is required in 'genericController' method");
    }
    ...
    if (options.hasOwnProperty("customRoutes")) {
        options.customRoutes.forEach((route) => {
            if (route.hasOwnProperty("middleware") && route.middleware.length > 0) {
                ...
                router[route.method.toLowerCase()](route.path, Controller[route.handler]);
            }
        });
    }
