...
async function getCustomer () {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/users/${useRoute().params.id}`, {
        method: 'GET',
        headers: {
...
async function getPaymentsForThisUser () {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/transactions/transaction/user/${useRoute().params.id}`, {
        method: 'GET',
        headers: {
            'Content-type': 'application/json'
        },
        ...
        <div class="p-4 lg:p-10" v-if="isLoading">
            <div class="md:flex md:flex-wrap md:justify-between font-light text-sm text-gray-400 mb-6">
                <div class="flex flex-wrap items-center">
                    <router-link :to="{ name: 'customers' }" class="bg-blue-100 text-blue-800 text-sm font-medium ml-2 px-2.5 py-0.5 rounded-full mr-2"><i class="fa-solid fa-chevron-left mr-2"></i> Retour</router-link>
        ...
        <div class="p-4 lg:p-10" v-if="!isLoading">
            ...
            <div class="flex flex-wrap justify-between border-b border-gray-200 pb-5">
                <p class="font-bold text-2xl flex flex-wrap items-center">
                    ...
                    <span :class="`${data.customer.role === 'merchant' ? 'bg-orange-100 text-orange-800' : data.customer.role === 'customer' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' } text-sm font-medium ml-2 px-2.5 py-0.5 rounded`">{{ data.customer.role === 'merchant' ? 'Marchant' : data.customer.role === 'customer' ? 'Client' : 'Admin' }}</span>
                </p>
            </div>
                ...
                <table class="font-light text-left">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="p-3 border-r"><i class="fa-brands fa-cc-visa mr-2"></i> <i class="fa-solid fa-ellipsis mr-2"></i> 4444</td>
                            <td class="p-3"><span class="bg-blue-100 text-blue-800 text-sm font-medium ml-2 px-2.5 py-0.5 rounded-full mr-2">{{ Object.keys(data.payments).length }}</span> Paiements réalisés</td>
                        </tr>
                    </tbody>
                </table>
            </div>
