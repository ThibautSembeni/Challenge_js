...
async function getTransaction () {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/transactions/${useRoute().params.reference}`, {
        method: 'GET',
        headers: {
        ...
        <div class="p-4 lg:p-10" v-if="isLoading">
            <div class="md:flex md:flex-wrap md:justify-between font-light text-sm text-gray-400 mb-6">
                <div class="flex flex-wrap items-center">
                    <router-link :to="{ name: 'payments' }" class="bg-blue-100 text-blue-800 text-sm font-medium ml-2 px-2.5 py-0.5 rounded-full mr-2"><i class="fa-solid fa-chevron-left mr-2"></i> Retour</router-link>
        ...
        <div class="p-4 lg:p-10" v-if="!isLoading">
            ...
            <div class="flex flex-wrap justify-between border-b border-gray-200 pb-5">
                <p class="font-bold text-2xl flex flex-wrap items-center">
                    <FormatEuro v-if="payment.amount && payment.currency" :price="payment.amount" :currency="payment.currency" />
                    <span class="font-light text-gray-400 uppercase ml-2">{{ payment.currency }}</span>
                    <span :class="`${payment.status === 'pending' ? 'bg-orange-100 text-orange-800' : payment.status === 'paid' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' } text-sm font-medium ml-2 px-2.5 py-0.5 rounded`">{{ payment.status === 'pending' ? 'En attente' : payment.status === 'paid' ? 'Réussi' : 'Échec' }}</span>
                </p>
                <button class="relative inline-flex items-center justify-center p-0.5 mb-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-cyan-500 to-blue-500 focus:outline-none">
                ...
                <table class="font-light text-left">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="p-3 border-r"><i class="fa-brands fa-cc-visa mr-2"></i> <i class="fa-solid fa-ellipsis mr-2"></i> {{ payment.billing_info.card_number.slice(-4) }}</td>
                            <td class="p-3"><span class="bg-orange-100 text-orange-800 text-sm font-medium ml-2 px-2.5 py-0.5 rounded-full mr-2">10</span> Risque normal</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <PaymentDetail title="Chronologie">
                <ol class="relative border-l border-gray-200 mt-4">
                    <li class="mb-7 ml-6">
                        <span class="absolute flex items-center justify-center w-6 h-6 bg-green-100 rounded-full -left-3">
                            <i class="fa-solid fa-circle-check text-green-600"></i>
                        </span>
                        <h3 class="mb-1">Paiement réussi</h3>
                        <time class="block mb-2 text-sm font-normal leading-none text-gray-400">{{ moment(payment.updated_at).format('LLLL') }}</time>
                    </li>
                    <li class="ml-6">
                        ...
                        <time class="block mb-2 text-sm font-normal leading-none text-gray-400">{{ moment(payment.created_at).format('LLLL') }}</time>
                    </li>
                </ol>
            </PaymentDetail>
                ...
                <div class="flex flex-wrap justify-between my-4">
                    <div class="w-full md:w-1/2">
                        <div class="flex flex-wrap">
                            ...
                            <PaymentDetailLine title="Date de paiement" :content="moment(payment.created_at).format('ll')" />
                            <PaymentDetailLine title="Net" content="à faire (affichage formatté)" />
                            <PaymentDetailLine title="Heure de paiement" :content="moment(payment.created_at).format('LT')" />
                            <PaymentDetailLine title="Statut" :content="payment.status === 'pending' ? 'En attente' : payment.status === 'paid' ? 'Réussi' : 'Échec'" />
                            <PaymentDetailLine title="Type de paiement" :content="payment.shipping_info.payment_type" />
                        </div>
                    </div>
                </div>
            </PaymentDetail>
                ...
                <div class="flex flex-wrap justify-between my-4">
                    <div class="w-full md:w-1/2">
                        <div class="flex flex-wrap">
                            ...
                            <PaymentDetailLine title="Numéro" :content="'... ' + payment.billing_info.card_number.slice(-4)" />
                            <PaymentDetailLine title="Empreinte" :content="payment.reference" />
                            <PaymentDetailLine title="Date d'expiration" :content="payment.billing_info.expiration_date" />
                            ...
                            <PaymentDetailLine title="Vérification CVC" :content="payment.billing_info.cvc ? 'Réussi' : 'En échec' " />
                ...
                <Table>
                    ...
                    <template #tbody>
                        <tr
                            ...
                            <td class="px-6 py-4">
                                <span class="bg-orange-100 text-orange-800 text-sm font-medium ml-2 px-2.5 py-0.5 rounded-full mr-2">10</span>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <span class="bg-orange-100 text-orange-800 text-sm font-medium ml-2 px-2.5 py-0.5 rounded">
