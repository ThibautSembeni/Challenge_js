<template>
  <div class="relative">
    ...
    <div
      ...
      :class="{
        hidden: !isOpen,
        'z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 mt-1': true,
        absolute: true
      }"
    >
    ...
    <Modal :open="showModal" @update:open="showModal = $event">
      ...
      <template #footer>
        <button
          @click="executeDelete"
          type="button"
          class="text-white bg-red-700 hover:bg-blue-800 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center"
        >
          Supprimer
        </button>
        <button
